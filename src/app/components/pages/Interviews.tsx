...
import { useGetApplication, useGetApplications } from "@/api/hooks/useApplication";
...
export function Interviews() {
    ...
    const applicationId = formValues.applicationId.trim();
    const { data: applicationsResponse } = useGetApplications({
        pageNumber: 1,
        pageSize: 200,
    });
    const applicationQuery = useGetApplication(applicationId);
    console.log(applicationQuery);

    const getCandidateNameFromApplication = (application: any) => {
    ...
    useEffect(() => {
        ...
        if (formValues.candidateName) return;
        const raw = applicationQuery.data as any;
        const applicationData = raw?.data ?? raw;
        const candidateName = getCandidateNameFromApplication(applicationData);
        if (!candidateName) return;
        setFormValues((prev) => ({ ...prev, candidateName }));
    }, [applicationId, applicationQuery.data, formValues.candidateName]);

    const applicationItems = useMemo(() => {
        const raw = applicationsResponse as any;
        const items =
            raw?.data?.items ??
            raw?.data ??
            raw?.items ??
    ...
    const handleApplicationSelect = (value: string) => {
        setFormValues((prev) => ({ ...prev, applicationId: value }));
        const selected = applicationItems.find((item: any) => String(item?.id) === value);
        if (!selected) return;
        const candidateName = getCandidateNameFromApplication(selected);
        if (candidateName) {
            setFormValues((prev) => ({ ...prev, candidateName }));
        }
    };
        ...
        <div className="p-8">
            ...
            <div className="flex gap-4 mb-6">
                <button
                    ...
                    className={`px-6 py-2 rounded-lg transition-colors ${selectedView === "upcoming"
                ...
                <button
                    ...
                    className={`px-6 py-2 rounded-lg transition-colors ${selectedView === "past"
                        ? "bg-teal-600 text-white"
                        : "bg-white text-gray-700 border border-gray-300 hover:bg-gray-50"
                        }`}
                >
                    ...
                    <form onSubmit={handleScheduleSubmit} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <select
                                ...
                                onChange={(event) =>
                                    handleApplicationSelect(event.target.value)
                                }
                            ...
                            >
                                ...
                                {applicationItems.map((app: any) => {
                                    const labelCandidate =
                                        getCandidateNameFromApplication(app) ||
                                        app?.email ||
                                        "Candidate";
                                    const labelJob = app?.jobTitle || app?.jobName || app?.jobId || "Job";
                                    return (
                                        <option key={app?.id ?? labelCandidate} value={String(app?.id)}>
                                            {labelCandidate} • {labelJob}
                                        </option>
                                    );
                                })}
                            </select>
